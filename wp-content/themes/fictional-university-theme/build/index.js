...
const googleMaps = new _modules_GoogleMap__WEBPACK_IMPORTED_MODULE_3__["default"]();
...
class MyNotes {
  ...
  deleteNote(e) {
    const myNotes = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li");
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      beforeSend: xhr => {
        xhr.setRequestHeader('X-WP-Nonce', univData.nonce);
      },
      url: `${univData.root_url}/wp-json/wp/v2/note/${myNotes.data('id')}`,
      type: 'DELETE',
...
class Search {
  ...
  typingLogic(event) {
    ...
    if (this.resultsDiv.html() !== '<div class="spinner-loader"></div>') this.resultsDiv.html('<div class="spinner-loader"></div>');
  ...
  async getResults() {
    ...
    //    `);
    // }
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(univData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val(), results => {
      var _results$generalInfo, _results$generalInfo2, _results$programs, _results$programs2, _results$campuses, _results$campuses2;
      ...
      this.resultsDiv.html(`
        <div class="row">
          <div class="one-third">
            <h2 class="search-overlay__section-title">General Information</h2>
            ${(_results$generalInfo = results.generalInfo) !== null && _results$generalInfo !== void 0 && _results$generalInfo.length ? '<ul class="link-list min-list">' : "<p>No general information matches that search.</p>"}
              ${(results.generalInfo || []).map(item => `<li><a href="${item.permalink}">${item.title}</a> ${item.postType == "post" ? `by ${item.authorName}` : ""}</li>`).join("")}
            ${(_results$generalInfo2 = results.generalInfo) !== null && _results$generalInfo2 !== void 0 && _results$generalInfo2.length ? "</ul>" : ""}
          </div>
          <div class="one-third">
            <h2 class="search-overlay__section-title">Programs</h2>
            ${(_results$programs = results.programs) !== null && _results$programs !== void 0 && _results$programs.length ? '<ul class="link-list min-list">' : `<p>No programs match that search. <a href="${univData.root_url}/programs">View all programs</a></p>`}
              ${(results.programs || []).map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${(_results$programs2 = results.programs) !== null && _results$programs2 !== void 0 && _results$programs2.length ? "</ul>" : ""}
          ...
          <div class="one-third">
            ...
            ${(_results$campuses = results.campuses) !== null && _results$campuses !== void 0 && _results$campuses.length ? '<ul class="link-list min-list">' : `<p>No campuses match that search. <a href="${univData.root_url}/campuses">View all campuses</a></p>`}
              ${(results.campuses || []).map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${(_results$campuses2 = results.campuses) !== null && _results$campuses2 !== void 0 && _results$campuses2.length ? "</ul>" : ""}
  ...
  keyPressDispatcher(event) {
    // s key
    if (event.keyCode === 83 && !this.isOpen && !jquery__WEBPACK_IMPORTED_MODULE_0___default()("input, textarea").is(":focus")) {
      this.openOverlay();
    } // esc key
